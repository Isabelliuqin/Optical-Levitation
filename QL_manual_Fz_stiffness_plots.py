...
grad_z0 = np.asarray(TQ.Fz_total_gradient(rho_0[0][0], rho_0[1], rho, n_0, n_s, w_0, w, z_R, P, target = 'reflective', integration_method = integration_method, grid_size = grid_size)['Fz_grad'])
...
rho_0z = np.asarray(TQ.Fz_total_gradient(rho_0[0][0], rho_0[1], rho, n_0, n_s, w_0, w, z_R, P, target = 'reflective', integration_method = integration_method, grid_size = grid_size)['rho_0z'])

#rho_0xratio = list(map(lambda x: x/w_0, rho_0x))


plt.figure(13)
plt.plot( rho_0z * 10 ** 6, grad_z0*10**8, lw=2, c="c", label="rho_0x = -7.07um")

plt.plot( rho_0z * 10 ** 6, grad_z1*10**8, lw=2, c="r", label="rho_0x = -5um")

plt.plot( rho_0z * 10 ** 6, grad_z2*10**8, lw=2, c="g", label="rho_0x = 5um")

plt.plot( rho_0z * 10 ** 6, grad_z3*10**8, lw=2, c="y", label="rho_0x = 7.07um")


new_ticks1 = np.linspace(0,1800,10) # plot axis
print(new_ticks1)
plt.xticks(new_ticks1,fontsize=20)
plt.yticks(np.linspace(-0.5, 1, 4),fontsize=20)
ax = plt.gca()
ax.spines['top'].set_color('none')
ax.spines['right'].set_color('none')
ax.xaxis.set_ticks_position('bottom')
...
MTP.table_parameter('x-axs w0 to 60', 'related to w', rho_0[1]* 10 ** 6, '-7.07, -5, 5, 7.07', '30')
