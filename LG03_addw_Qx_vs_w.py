# -*- coding: utf-8 -*-
"""
Created on Mon Aug 31 17:17:52 2020
...
w = np.linspace(w_0, 2*np.sqrt(2/3) *rho, 100)
...
plt.figure(9)
plt.plot( np.sqrt(3/2)*w / (rho), Q_x0, lw=2, c="c", label="rho_0x/rho = 0.05")

plt.plot( np.sqrt(3/2)*w  / (rho), Q_x1, lw=2, c="r", label="rho_0x/rho = 0.25")

plt.plot( np.sqrt(3/2)*w  / (rho), Q_x2, lw=2, c="g", label="rho_0x/rho = 0.5")

plt.plot( np.sqrt(3/2)*w  / (rho), Q_x3, lw=2, c="y", label="rho_0x/rho = 0.75")

plt.plot( np.sqrt(3/2)*w  / (rho), Q_x4, lw=2, c="b", label="rho_0x/rho = 1")

plt.plot( np.sqrt(3/2)*w  / (rho), Q_x5, lw=2, c="k", label="rho_0x/rho = 1.25")

plt.plot( np.sqrt(3/2)*w  / (rho), Q_x6, lw=2, c="m", label="rho_0x/rho = 2")
...
plt.yticks(np.linspace(-1.0, 0.1, 12),fontsize=20)
...
###############################################
rho_0x = np.linspace(0.1*rho,2*rho,100)
rho_0 = [0,0]
...
plt.plot(rho_0x/rho, np.array(Qoplist), lw=2, c="c", label="w = rho")
